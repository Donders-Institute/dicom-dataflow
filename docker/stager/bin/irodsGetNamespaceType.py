...
if __name__ == "__main__":
    ...
    parg = argparse.ArgumentParser(description='determine data type (file, collection) referred by a given iRODS namespace')

    parg.add_argument('namespace',
                      metavar = 'namespace',
                      type = str,
                      help = 'specify the iRODS namespace')
    ...
    rule_fpath = os.path.join(os.path.join(os.environ['PYTHON_LIBDIR'], './donders/rdm/rules', 'getNamespaceType.r'))

    out = irdm.__rdm_exec_rule__(irule_script=rule_fpath, inputs={'namespace': args.namespace})
